...
# EnvSetup
KBUILD_BUILD_USER="renaldigp"
KBUILD_BUILD_HOST="ryujinz"
...
clone() {
	echo " "
	msg "|| Cloning Clang ||"
	git clone --depth=1 https://github.com/Ryujinz/Ryujinz-Clang.git clang-llvm --no-tags --single-branch

		# Toolchain Directory defaults to clang-llvm
	TC_DIR=$KERNEL_DIR/clang-llvm

	msg "|| Cloning Anykernel ||"
	git clone --depth 1 --no-single-branch https://github.com/Ryujinz/anykernel.git -b main
}
...
build_kernel() {
    ...
    make O=out $DEFCONFIG CC=clang AR=llvm-ar OBJDUMP=llvm-objdump STRIP=llvm-strip OBJCOPY=llvm-objcopy LD=ld.lld CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi-

	msg "|| Started Compilation ||"
	BUILD_START=$(date +"%s")
	make -j"$PROCS" O=out CC=clang AR=llvm-ar OBJDUMP=llvm-objdump STRIP=llvm-strip OBJCOPY=llvm-objcopy LD=ld.lld CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi-
	BUILD_END=$(date +"%s")
